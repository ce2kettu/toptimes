...
function Leaderboard.addRecord(player, time)
	...
	dbQuery(Leaderboard.callback, {arenaElement, false, account}, connection, query2)
	
end


function Leaderboard.callback(handle, arenaElement, isInitialFetch, account)
	...
	for i = 1, Leaderboard.ROWS_TO_SHOW, 1 do
		...
		if not Leaderboard.oldCache[arenaElement] and not result[i] then return end
		...
		elseif Leaderboard.oldCache[arenaElement][i].time ~= result[i].time_ms then
			...
			Leaderboard.fetchPersonalBestTime()
...
function Leaderboard.fetchPersonalBestTime()
...
addEventHandler("onPlayerJoinArena", root, Leaderboard.fetchPersonalBestTime)
...
function Leaderboard.clientRequest()
	...
	if account then
		...
		personalBest = Leaderboard.personalBestCache[arenaElement] and Leaderboard.personalBestCache[arenaElement][account] or false
		
	end

	triggerClientEvent(arenaElement, "onClientTopTimesUpdate", arenaElement, Leaderboard.cache[arenaElement], personalBest)
